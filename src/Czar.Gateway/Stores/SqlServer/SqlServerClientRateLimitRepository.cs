...
namespace Czar.Gateway.Stores.SqlServer
{
    ...
    public class SqlServerClientRateLimitRepository : IClientRateLimitRepository
    {
        ...
        public SqlServerClientRateLimitRepository(CzarOcelotConfiguration option)
        {
            _option = option;
        }
        ...
        /// <returns></returns>
        public async Task<(bool RateLimit, List<CzarClientRateLimitOptions> rateLimitOptions)> CheckClientRateLimitAsync(string clientid, string path)
        {
            using (var connection = new SqlConnection(_option.DbConnectionStrings))
            {
                ...
                var result = (await connection.QueryAsync<CzarClientRateLimitOptions>(sql, new { clientid, path }))?.AsList();
